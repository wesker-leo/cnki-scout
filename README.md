...
需要 Python 3.10+，以及可访问知网的校园网、学校 VPN 或机构账号。以下命令适用于 macOS/Linux：
...
先在弹出的浏览器里**手动**完成机构登录，回到终端按 Enter：
...
默认相邻访问间隔 8–15 秒，允许的最小间隔为 5 秒；一次最多保存 200 篇、扫描 50 页。程序不会自动识别验证码、轮换代理或并发访问。遇到安全验证、403/429/503 或翻页异常会停止并保存已有结果。浏览器会话在本地 `.browser_profile/`，原始采集数据在 `output/`；两者均被 `.gitignore` 排除，**不要上传或分享浏览器会话**。请遵守[知网会员服务使用协议](https://wap.oversea.cnki.net/cn/member/agreement.html)和所在机构的访问规定。
